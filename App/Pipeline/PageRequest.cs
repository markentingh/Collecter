...
namespace Collector.Pipeline
{
    public class PageRequest
    {
        ...
        {
            //the Pipeline.PageRequest is simply the first page request for a Collector website. 

            S = new Core(server, context);

            var path = context.Request.Path.ToString().Substring(1).Split('?', 2)[0].Split('/');

            var page = GetWebPage(("Collector.Pages." + (path[0] == "" ? "Home" : S.Util.Str.Capitalize(path[0].Replace("-"," ")).Replace(" ",""))));
